...
class ViewController: UIViewController {
    ...
    @objc func updateUI(){
        if let scheduledAlarm = Alarm.scheduled {
            let formattedAlarm = scheduledAlarm.date.formatted(.dateTime.day(.defaultDigits)
                                                                .month(.defaultDigits)
                                                                .year(.twoDigits)
                                                                .hour().minute())
    ...
    func presentNeedAuthorizationAlert(){
        ...
        let message = "Alarms don't work without notifications. Please go to the iOS settings app and grant us notification permissions."
